...
public class ListSoldPropertiesController {
    ...
    public List<Property> askAboutStore(List<Property> properties, Scanner sc, StoreRepository storeList){
        int numOfOptions = storeList.getSize();
        int option;
        boolean valid = false;
        VerifyInput verifyInput = new VerifyInput();
        do{
        ...
        if(option != 0){
            EditAndSortLists editAndSortLists = new EditAndSortLists();
            properties = editAndSortLists.getPropertiesByXStore(properties, storeList.getStores().get(option-1).getEmail());
        }
        return properties;
    }
